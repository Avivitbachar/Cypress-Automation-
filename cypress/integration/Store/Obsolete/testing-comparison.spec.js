...
describe ('First test', ()=>{
    ...
    it('Launch + spin + check history', ()=>{
        ...
        cy.get('.cy-gaming-history-bankroll-value').first().invoke('text').then(balance=>{
            cy.log(balance)
        })
        ...
        cy.get('.cy-gaming-history-bankroll-value').first().invoke('text').then(newB=>{
            cy.log(newB)
            expect(newB).to.not.eq(balance)
        })
