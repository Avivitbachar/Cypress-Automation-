...
describe('First test', () => {
    ...
    it('Launch + spin + check history', () => {
      ...
      cy.wait(10000)
      cy.reload()
      cy.get('.cgp-provider-play-tech').first().find('.cgp-game-details').then(compare=>{
          let balance = compare.find('.cy-gaming-history-bankroll-value').text()
          let winnings = compare.find('.cy-gaming-history-winning-value cgp-themes-main-text').text()
          if(balance === '@firstBalance'){
              expect(winnings).to.eq('$0.01')
          }else{
            expect(balance).to.not.eq('@firstBalance')
          }
